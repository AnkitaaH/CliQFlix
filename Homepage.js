...
const apiPaths = {
    ...
    searchOnYoutube: (query) => `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query}&key=AIzaSyAX64hD8t0GlJp9qImldKpbTutjIF10UBQ`
}
...
function searchMovies() {
    ...
    const searchUrl = `${apiEndpoint}/search/movie?api_key=${apikey}&query=${searchInput}`;
  
    fetchAndBuildMovieSections(searchUrl, 'Search Results')
      .then(list => {
  ...
  function displaySearchResults(list) {
    ...
    const noResultsHTML = '<p>No results found.</p>';
